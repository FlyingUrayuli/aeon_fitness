  ...
  <div class="bg-gray-600 text-white p-8 rounded-lg shadow-lg mb-8">
    ...
    <div v-if="type === 'title'" class="text-center">
      <div class="flex justify-center items-center mb-6">
        ...
        <h1 class="text-4xl font-bold font-stzongsong">{{ data.name }}</h1>
      </div>
    ...
    <div v-else-if="type === 'model'" class="max-w-6xl mx-auto">
      <div class="flex items-center mb-6">
        ...
        <h2 class="text-3xl font-bold font-stzongsong">3D 模型展示</h2>
      </div>
    ...
    <div v-else-if="type === 'cta'" class="text-center">
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        <button class="bg-blue-300 hover:bg-blue-600 text-white font-bold py-3 px-8 rounded-lg transition-colors duration-300 flex items-center justify-center">
          ...
          <RouterLink to="/about">立即諮詢</RouterLink>
        </button>
        <button
          @click="addToCart"
          class="cursor-pointer bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-lg transition-colors duration-300 flex items-center justify-center"
        >
          <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// In ProductDetailSection.vue script
...
const addToCart = () => {
  router.push({ path: '/cart', query: { product: props.data.id } })
}
</script>
